...
下載:<br>
如果我們不是使用SCM輪循的方式，而是採用url觸發方式時，<br>
可以使用git來進行下載最新版的程式碼(請看下方黑色區塊的git)。<br>
<br>
例如: 我們的CI Server沒有真實的對外IP，或許就用輪循讓他自己抓。<br>
<br>
組態:<br>
非常簡單，我把測試環境使用的資料及資料庫等設定，放入了.env.example內，這樣就可以給CI Server直接拿來使用囉。<br>
<br>
啟動:<br>
這裡很單純的使用php內建的開發用主機，進行Laravel的啟動<br>
<br>
測試:<br>
因為Laravel框架，我們要在CI測試操簡單，就直接給他跑下去就對了。<br>
phpunit及php artisan dusk.<br>
<br>
部署:<br>
如果都沒有問題，就可以執行部署到遠端主機了。<br>
例如透過rsync，使用加密方式進行差異檔案的更新，<br>
再大的Project基本上也可以在幾秒內完成。<br>
